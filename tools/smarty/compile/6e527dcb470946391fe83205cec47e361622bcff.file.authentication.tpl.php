<?php /* Smarty version Smarty-3.0.7, created on 2012-02-02 23:30:21
         compiled from "/home/a2012616/public_html/themes/theme138/authentication.tpl" */ ?>
<?php /*%%SmartyHeaderCode:1619143574f2b006d2564e4-31130231%%*/if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    '6e527dcb470946391fe83205cec47e361622bcff' => 
    array (
      0 => '/home/a2012616/public_html/themes/theme138/authentication.tpl',
      1 => 1327934616,
      2 => 'file',
    ),
  ),
  'nocache_hash' => '1619143574f2b006d2564e4-31130231',
  'function' => 
  array (
  ),
...
.</h4>
			<p class="text">
				...
				<input type="text" id="email_create" name="email_create" value="<?php if (isset($_POST['email_create'])){?><?php echo stripslashes(smarty_modifier_escape($_POST['email_create'],'htmlall','UTF-8'));?>
...
<?php }?>" class="account_input" />
			...
			<p class="text">
				...
				<input type="password" id="passwd" name="passwd" value="<?php if (isset($_POST['passwd'])){?><?php echo stripslashes(smarty_modifier_escape($_POST['passwd'],'htmlall','UTF-8'));?>
<?php }?>" class="account_input" />
			</p>
			<p class="submit">
				<?php if (isset($_smarty_tpl->getVariable('back',null,true,false)->value)){?><input type="hidden" class="hidden" name="back" value="<?php echo smarty_modifier_escape($_smarty_tpl->getVariable('back')->value,'htmlall','UTF-8');?>
			...
			<fieldset>
				...
				<div id="opc_account_form" style="display: block; ">
					...
					<p class="required text">
						...
						<input type="text" id="guest_email" name="guest_email" value="<?php if (isset($_POST['guest_email'])){?><?php echo $_POST['guest_email'];?>
<?php }?>">
						<sup>*</sup>
					</p>
					...
					<p class="required text">
						...
						<input type="text" id="firstname" name="firstname" onblur="$('#customer_firstname').val($(this).val());" value="<?php if (isset($_POST['firstname'])){?><?php echo $_POST['firstname'];?>
<?php }?>">
...
<?php }?>">
					...
					<p class="required text">
						...
						<input type="text" id="lastname" name="lastname" onblur="$('#customer_lastname').val($(this).val());" value="<?php if (isset($_POST['lastname'])){?><?php echo $_POST['lastname'];?>
<?php }?>">
...
<?php }?>" />
					...
					<div id="vat_number" style="display:none;">
						<p class="text">
							...
							<input type="text" name="vat_number" value="<?php if (isset($_POST['vat_number'])){?><?php echo $_POST['vat_number'];?>
<?php }?>" />
						</p>
					</div>
					<p class="required text">
						...
						<input type="text" name="address1" id="address1" value="<?php if (isset($_POST['address1'])){?><?php echo $_POST['address1'];?>
<?php }?>">
						<sup>*</sup>
					</p>
					<p class="required postcode text">
						...
						<input type="text" name="postcode" id="postcode" value="<?php if (isset($_POST['postcode'])){?><?php echo $_POST['postcode'];?>
...
" <?php if (($_smarty_tpl->getVariable('sl_country')->value==$_smarty_tpl->tpl_vars['v']->value['id_country'])){?> selected="selected"<?php }?>><?php echo smarty_modifier_escape($_smarty_tpl->tpl_vars['v']->value['name'],'htmlall','UTF-8');?>
					...
					<p class="text required">
						...
						<input type="text" name="phone" id="phone" value="<?php if (isset($_POST['phone'])){?><?php echo $_POST['phone'];?>
...
" method="post" id="account-creation_form" class="std">
	...
	<fieldset class="account_creation">
		...
		<p class="required text">
			...
			<input onkeyup="$('#firstname').val(this.value);" type="text" id="customer_firstname" name="customer_firstname" value="<?php if (isset($_POST['customer_firstname'])){?><?php echo $_POST['customer_firstname'];?>
<?php }?>" />
			<sup>*</sup>
		</p>
		<p class="required text">
			...
			<input onkeyup="$('#lastname').val(this.value);" type="text" id="customer_lastname" name="customer_lastname" value="<?php if (isset($_POST['customer_lastname'])){?><?php echo $_POST['customer_lastname'];?>
<?php }?>" />
			<sup>*</sup>
		</p>
		<p class="required text">
...
<?php }?>" />
		...
		<p class="text">
			...
			<input type="text" name="address2" id="address2" value="<?php if (isset($_POST['address2'])){?><?php echo $_POST['address2'];?>
<?php }?>" />
		</p>
		<p class="required postcode text">
			<label for="postcode"><?php echo smartyTranslate(array('s'=>'Zip / Postal Code'),$_smarty_tpl);?>
...
<?php }?></textarea>
		...
		<p class="required"><?php echo smartyTranslate(array('s'=>'You must register at least one phone number'),$_smarty_tpl);?>
 <sup>*</sup></p>
		<p class="text">
...
<?php }?>" />
		...
		<p class="text">
			...
			<input type="text" name="phone_mobile" id="phone_mobile" value="<?php if (isset($_POST['phone_mobile'])){?><?php echo $_POST['phone_mobile'];?>
<?php }?>" />
		</p>
		<p class="required text" id="address_alias">
			<label for="alias"><?php echo smartyTranslate(array('s'=>'Assign an address title for future reference'),$_smarty_tpl);?>
 !</label>
			<input type="text" name="alias" id="alias" value="<?php if (isset($_POST['alias'])){?><?php echo $_POST['alias'];?>
