...
<table id="product_comparison">
	...
	{foreach from=$products item=product name=for_products}
		...
		<td width="{$width}%" class="ajax_block_product">
			<h5><a class="product_link" href="{$product->getLink()}" title="{$product->name|truncate:30:'...'|escape:'htmlall':'UTF-8'}">{$product->name|truncate:25:'...'|escape:'htmlall':'UTF-8'}</a></h5>
			...
			<a href="{$product->getLink()}" title="{$product->name|escape:html:'UTF-8'}" class="product_image" >
				<img src="{$link->getImageLink($product->link_rewrite, $product->id_image, 'medium')}" alt="{$product->name|escape:html:'UTF-8'}" />
			</a>
